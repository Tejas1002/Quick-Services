...
<body>
    ...
    <section class="hero">
        <div class="swiper mySwiper">
            <div class="swiper-wrapper">
                <div class="swiper-slide" style="background-image: url('https://images.unsplash.com/photo-1521207418485-99c705420785?q=80&w=2670&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D');"></div>
                <div class="swiper-slide" style="background-image: url('https://images.unsplash.com/photo-1553265381-674034b34554?q=80&w=2780&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D');"></div>
                <div class="swiper-slide" style="background-image: url('https://img.freepik.com/free-photo/side-view-man-working-as-plumber_23-2150746307.jpg?t=st=1742837075~exp=1742840675~hmac=34b9ab7fe81c9104d0b8278dd33ac8b85340d6afc9c595446bebb1ddaea75b90&w=1800');"></div>
                <div class="swiper-slide" style="background-image: url('https://img.freepik.com/free-photo/worker-repairing-water-heater_23-2149334230.jpg?t=st=1742837134~exp=1742840734~hmac=5ccd19b90b1c4329e55bd780e3f268848bc2344f3b6c46b28acb5df44b141ac2&w=1800');"></div>
            </div>
            <div class="swiper-pagination"></div>
        </div>
        <div class="hero-content">
            <h1>Plumber Services</h1>
            <p>Reliable Plumbing Solutions for Your Home or Office</p>
        </div>
    </section>
    ...
    <script>
        ...
        function filterProducts(page = 1) {
            ...
            $.ajax({
                url: '{{ route("plumber.filter.products") }}',
                method: 'GET',
        ...
        function bindLikeEvents() {
            $('.like-product').off('click').on('click', function() {
                ...
                $.ajax({
                    url: '{{ route("plumber.product.like") }}',
                    method: 'POST',
        ...
        function bindWishlistEvents() {
            $('.wishlist-product').off('click').on('click', function() {
                ...
                $.ajax({
                    url: '{{ route("plumber.wishlist.add") }}',
                    method: 'POST',
                    data: { product_id: productId, _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        if (response.success) {
        ...
        function updateSavedItemsCount() {
            @if(auth()->check())
                $.ajax({
                    url: '{{ route("plumber.saved.items.count") }}',
                    method: 'GET',
                    success: function(response) {
